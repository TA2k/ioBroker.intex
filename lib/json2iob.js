  ...
  async parse(path, element, options) {
    try {
      ...
      this.adapter.log.error(
        "Error extract keys: " + path + " " + JSON.stringify(element),
      );
      this.adapter.log.error(error);
    }
  }
  extractArray(element, key, path, options) {
    try {
      ...
      element.forEach(async (arrayElement, index) => {
        ...
        if (
          options.preferedArrayName &&
          arrayElement[options.preferedArrayName]
        ) {
          arrayPath = arrayElement[options.preferedArrayName].replace(
            /\./g,
            "",
          );
        }
        ...
        if (
          ...
          const subName =
            Object.keys(arrayElement)[0] + " " + Object.keys(arrayElement)[1];
          if (key) {
            subKey = key + "." + subKey;
          }
